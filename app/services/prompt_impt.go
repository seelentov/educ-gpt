...
func (p PromptServiceImpl) GetProblems(count int, topic string, theme string, userThemeStats *models.Theme, userAllStats []*models.Theme) string {
	prompt1 := fmt.Sprintf(". Подготовь %v задач по теме: %s.%s , в этих задачах должны быть как задачи на решение с помощью написания кода, так и теоретические. Количество теоретических и практических вопросов определи сам, возможно для этой теме нет возможности создать практическую задачу.", count, topic, topic)
...
func (p PromptServiceImpl) VerifyAnswer(problem string, answer string, language string) string {
	...
	prompt4 := ". Задача выполнена на " + language + ". Учти это при ответе. Ты не проверяешь грамматику, не качество структуры и не качество текста, ты проверяешь ответ на задачу."
