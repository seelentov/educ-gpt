...
func (p PromptServiceImpl) GetThemes(topic string, existedThemes []*models.Theme, userStats []*models.Theme) (string, error) {
	...
	prompt5 := `Отсортируй разделы по сложности: от простых к сложным. Пример ответа: [\"Основы синтаксиса\",\"Работа с функциями\", ...]. Разделов должно быть не менее 50 всего и они должны охватывать как начинающий уровень, так и продвинутый и более высокий по сложности. В ответе должен быть только НЕ ПУСТОЙ массив JSON со строками!`
