...
func (p PromptServiceImpl) GetThemes(topic string, existedThemes []*models.Theme, userStats []*models.Theme) (string, error) {
	...
	getThemesPrompt5 := `Отсортируй разделы по сложности: от простых к сложным. Пример ответа: [\"Основы синтаксиса\",\"Работа с функциями\", ...]. Разделов должно быть не менее 30 всего. В ответе должен быть только НЕ ПУСТОЙ массив JSON со строками!`
