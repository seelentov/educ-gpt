...
func (p PromptServiceImpl) GetTheme(topic string, theme string, userThemeStats *models.Theme, userAllStats []*models.Theme) string {
	...
	prompt6 := ` Ответ должен быть json в виде {\"text\": \"<теоретический текст>\", \"problems\":[\"<задача>\", \"<задача>\", ...]}. Если тема не связанна с конкретной технологией или языком программирования, то технология или язык программирования может быть любым. В ответе должен быть только JSON объект с text, который будет включать основной ответ и problems с задачами в виде строк, без нумерации!`
